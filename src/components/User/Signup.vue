          ...
          <v-card-text>
            <v-container>
              <form @submit.prevent="onSignUp">
                ...
                <v-layout row>
                  <v-flex xs12>
                    <v-text-field name="confirmPassword" label="Confirm Password" id="confirmPassword" type="password" required v-model="confirmPassword" :rules="[comparePasswords]"></v-text-field>
                  </v-flex>
                </v-layout>
                <v-layout row>
                  <v-flex xs12>
...
export default {
  ...
  methods: {
    onSignUp () {
      ...
      this.$store.dispatch('userSignUp', newUser)
    },
    onDissmised () {
